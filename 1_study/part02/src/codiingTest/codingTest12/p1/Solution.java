...
public class Solution {
    public static int solution(int n, int num) {
        ...
        for (int i = 0; i < n; i++) {               // num 값을 String 으로 받는 변수
            long[] arr = countNum(number);           // 각 자리 숫자를 기록하는 함수
            number = makeNum(arr);                  // 위에서 만든 arr 배열을 토대로 String number 를 업데이트
        }

        return (int) (Long.parseLong(number) % 10004);    // 10004 로 나눈 결과값 반환
    }

    public static long[] countNum(String num) {      // 각자리 숫자 갯수 세는 함수
    ...
    public static String makeNum(long[] arr) {       // arr 에 기록된 값을 숫자로 변경하는 함수
